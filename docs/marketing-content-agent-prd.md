...
以 Hermes Agent 的对话为唯一用户入口。Agent 根据用户在对话中提供的链接、附件、文本或知识库引用，动态调用 MCP/已接入资料读取能力获得本次营销上下文，再串联已有开源 Skills：热点聚合、内容策略、文案、创意、封面/图文卡片、公众号排版与草稿创建。用户只在选题、内容和发布等业务判断节点确认；Agent 将每次执行记录为一个 Campaign Run，返回选题、文案、图文和渠道发布包。
...
| 场景描述 | 在 Hermes Agent 对话中围绕一个楼宇/项目，借安全热点生成并交付多渠道营销内容 |
...
- **经过**：用户在对话中给出项目资料/链接/知识库引用 → Agent 动态读取上下文 → 获取热点 → 生成 3 个选题 → 人工确认 → 生成文案与图文 → 输出渠道包。
...
| 环节 | 采用能力 | 本期定位 | 不承担的职责 |
| --- | --- | --- | --- |
| 热点 | `news-aggregator-skill` | 聚合社会热榜与政府/政策来源并输出结构化信号 | 不判断楼宇匹配度 |
...
| 2 | 热点与政策信号采集 | 聚合热点、政府政策、标准化、去重、风险过滤 | P0 |
...
**本期包含**：对话式任务入口、Agent 动态读取上下文、Skill 编排、热点采集、审核前选题、文案与图文、公众号草稿、导出包、审计记录。
...
| 系统/组件 | 关系类型 | 本期动作 | 责任方 |
| --- | --- | --- | --- |
| Hermes Agent | 对话与编排入口 | 从对话识别资料来源，调用 MCP/工具/Skills，持久化 Campaign Run | Agent/后端团队 |
| 现有知识库 | 可选上下文来源 | 通过已接入 MCP 或工具返回检索结果 | 知识库团队 |
| 开源 Skills | 业务能力 | 安装、配置、按顺序调用 | Agent 团队 |
| 热点与政府数据源 | 外部来源 | 由热点 Skill 访问并保留原始链接、发布日期和来源级别 | Agent 团队 |
...
- [上海市人民政府](https://www.shanghai.gov.cn/) 与 [上海市发展改革委政策文件](https://fgw.sh.gov.cn/fgw_zcwjfl/index.html)：首期政府信号白名单来源；其他部门和区级来源由业务在 D1 确认。
...
**交互方式**：用户在 Hermes Agent 对话中描述任务，并可直接给出飞书/网页链接、文件附件、文本资料或知识库引用。Agent 根据上下文自动调用已接入的 MCP 或工具读取资料；不要求用户先进入知识库页面，也不要求在营销流程中指定固定读取 Skill。
...
| `fact_package` | 是 | 由 Agent 从本次对话上下文归一化而来，包含项目名、城市、商圈、交通、联系人、卖点、来源 |
...
| F-05 | 触发条件 | 上下文不足以形成事实包时，Agent 只追问缺失字段，不进入热点和内容生成。 |

#### 10.2.2 热点与政府政策信号采集

| 功能 | 要求 |
| --- | --- |
| 社会热榜来源 | `news-aggregator-skill` 配置微博、36氪、腾讯新闻等；可选接入 DailyHotApi。 |
| 政府来源 | 国家层面政策发布；上海市政府、上海市发展改革委、住建/规划资源/商务/投促和目标区政府官网的公开栏目。 |
| 来源级别 | `official_policy`、`official_news`、`social_trend` 三类，政府来源必须为前两类。 |
| 过滤 | 高风险政治、灾害、伤亡、犯罪、医疗、纠纷热点默认剔除；政策文件不作为“娱乐热点”使用。 |
| 追溯 | 保存标题、来源、原始链接、抓取时间、发布日期、政策发文机构、热度/政策相关性分。 |
| 输出分流 | 社会热榜输出“内容灵感”；政府来源输出“政策/城市机会信号”，仅在与项目事实相关且审核通过时进入营销选题。 |
...
1. 选题必须同时满足“信号低风险、与项目事实自然相关、目标人群可感知”三项。
2. 使用政府来源时，文案必须标注政策原始链接供审核；不得把政策标题改写为夸张营销承诺。
3. 人工必须选择一个选题后才可进入文案生成。
4. 任何“最低价、保证成交、全城最佳”等不可证明表述必须拦截。
5. 公众号与小红书文案不可简单复制，需按渠道长度与语气改写。
...
| 工作日 | 交付物 | 验收人/门禁 |
| --- | --- | --- |
| D1 | 确认 5 个核心 Skills、渠道账号、项目资料模板、政府来源白名单、验收样本 | 业务负责人确认范围 |
| D2 | 安装并验证热点、策略、文案、图像、公众号 Skills 的运行环境 | 技术负责人确认依赖可运行 |
| D3 | 完成对话上下文读取、事实包归一化与禁用词/来源规则 | 业务提供 3 份真实项目资料或可访问链接 |
| D4 | 跑通社会热榜与政府政策信号采集、风险过滤、原始链接审计 | 运营抽检 20 条信号，含至少 5 条政府来源 |
...
| TBD-5 | 确定社会热榜、政府来源白名单和风险词规则 | 10.2.2 | [TODO: 内容审核人] | D1 | 待定 |
