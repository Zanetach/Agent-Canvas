...
如果 Codex 生图由 Agent 管理，Agent 插件应按[插件协议](InUx-Canvas-1.0.2-extracted/integrations/AGENT-PLUGIN-PROTOCOL.md)提供本机 `/v1/image` 网关，并在启动环境中声明能力：

```bash
export BEEMAX_AGENT_GATEWAY_URL='http://127.0.0.1:19000'
export BEEMAX_AGENT_MODELS_JSON='{"text":[],"image":["codex-image"],"video":[]}'
```

这两个变量由 Hermes、Zylos 或其他 Agent 插件在启动时读取；`./install-agent.sh codex` 不会单独注册网关。应先安装对应 Agent 插件并重新启动 Agent 会话，插件随后会向 Bridge 注册模型。API Key 和认证信息继续由 Agent 管理。
