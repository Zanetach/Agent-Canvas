...
- Codex：Bridge 内置适配，复用 Codex 登录状态，不要求额外 Key。
...
- 其他 Agent：实现同一个注册请求即可，无需修改 Canvas 核心。

Hermes 与 Zylos 适配器会在插件生命周期读取 Agent 部署环境提供的 `BEEMAX_AGENT_GATEWAY_URL`、`BEEMAX_AGENT_MODELS_JSON` 和可选的 `BEEMAX_AGENT_INSTANCE_ID`，自动完成注册。它们是 Agent 插件与本机网关之间的内部交接信息，不是模型 API 地址或 Key。
