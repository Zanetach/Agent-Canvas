...
BeeMax Bridge 启动时会自动读取 Hermes 当前默认与备用 LLM，并与可用的 Codex、Agent 网关模型合并显示为托管 Agent Provider。Canvas 的文本节点调用对应 Agent，图片节点调用所选图片 Provider；模型凭证仍由各自运行时管理，不会写入 Canvas 的 AI 配置。切换 Hermes 模型后需重启 BeeMax Bridge。

如果 Hermes 环境还提供自己的文本、生图或视频网关，只需实现 `GET /v1/manifest` 并设置 `BEEMAX_AGENT_GATEWAY_URL`。插件会调用 `discover_agent_capabilities` 自动同步三类模型，不再需要复制模型清单。旧宿主仍可调用 `register_agent_capabilities`。两种方式都不包含 API Key，Canvas 只保存非敏感能力清单。
